...
public class IDGenerator {
    ...
    public static String generateFeeID(Connection connection, TransactionDAO transactionDAO) {
        String feeID = (char) (random.nextInt(26) + 'A') + (char) (random.nextInt(26) + 'A')
            + String.valueOf(random.nextInt(88_999_999) + 10_000_000);

        while(true) {
            boolean check = transactionDAO.doesTransactionExists(connection, feeID);
            if (check) feeID = (char) (random.nextInt(26) + 'A') + (char) (random.nextInt(26) + 'A')
                + String.valueOf(random.nextInt(88_999_999) + 10_000_000);
            else break;
        }
        return feeID;
    }
    public static String generateTransactionID(String feeID) {
        return feeID.charAt(1) + feeID.charAt(0) + feeID.substring(2);
    }
}
